...
void main()
{
  ...
  float load_percent_now, temp, lambda,k,p,r;
  ...
  printf("Enter lambda: ");    //number of runs for each beta value
  scanf("%f", &lambda);

  printf("Enter k: ");    //number of runs for each beta value
  scanf("%f", &k);
  ...
  int rt=1;
    ...
    {   
        ...
        while(r==1.) {r=(double)rand() / (double)RAND_MAX;}
        thresold_list[i] = pow(-1*pow(lambda,k)*log(1-r),1./k);
    ...
    float inv_cri_stress=bijun_cri_finding(1.0, 1.0, num_fib, thresold_list);
    ...
    sprintf(line2_p1, "%f", lambda);
    fputs(line2_p1, filePointer3);

    fputs(" ", filePointer3);

    sprintf(line2_p1_1, "%f", k);
