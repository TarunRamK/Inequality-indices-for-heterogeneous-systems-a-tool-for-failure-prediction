...
void main()
{
  ...
  float load_percent_now, temp, mean_value, sd;
  ...
  printf("Enter mean value: ");    //number of runs for each beta value
  scanf("%f", &mean_value);

  printf("Enter range on each side: ");    //number of runs for each beta value
  scanf("%f", &sd);
  ...
  int rt=1;
    ...
    {   
        thresold_list[i] = (mean_value - sd) + 2*sd*((double)rand() / (double)RAND_MAX);
    ...
    float inv_cri_stress=bijun_cri_finding(mean_value, sd, num_fib, thresold_list);
    ...
    float inv_stress=load_percent; // removed this part from end: "*inv_cri_stress" amount of load to test the model
    ...
    sprintf(line2_p1, "%f", mean_value);
    fputs(line2_p1, filePointer3);

    fputs(" ", filePointer3);

    sprintf(line2_p1_1, "%f", sd);
    fputs(line2_p1_1, filePointer3);

    fputs(" ", filePointer3);

     sprintf(line2_p1_2, "%f", inv_cri_stress);
    fputs(line2_p1_2, filePointer3);
